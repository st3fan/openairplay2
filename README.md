...
| `--tui-listen ADDR` | `OPENAIRPLAY2_TUI_LISTEN` | Also serve the [now-playing WebSocket](#now-playing-display) over TCP for a display on another machine, e.g. `127.0.0.1:7392`. A non-loopback address is advertised over mDNS as `_openairplay2._tcp` (unless `--no-avahi`), so displays find it on their own | off |
